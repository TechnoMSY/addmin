...
<div class="mt-16 p-6 max-w-4xl mx-auto w-full">
    ...
    <?php if (!empty($pesan_sukses)): ?>
        <div class="mb-4 p-4 bg-emerald-100 border border-emerald-300 text-emerald-800 rounded-lg text-sm flex items-center gap-2">
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500">Lihat data lama di sebelah kiri dan masukkan data baru di sebelah kanan untuk memperbarui akun.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm h-fit space-y-4">
            ...
            <div>
                ...
                <p class="text-sm font-semibold text-gray-800"><?php echo htmlspecialchars($data_user['nama'] ?? '-'); ?></p>
            </div>
            
            <div>
                <span class="text-xs text-gray-500 block">Username Akun:</span>
                <p class="text-sm font-semibold text-gray-800">@<?php echo htmlspecialchars($data_user['username'] ?? '-'); ?></p>
            </div>
            
            <div>
                <span class="text-xs text-gray-500 block">Alamat Email:</span>
                <p class="text-sm font-semibold text-gray-800"><?php echo htmlspecialchars($data_user['email'] ?? '-'); ?></p>
            </div>
        </div>
        ...
        <div class="md:col-span-2 bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            ...
            <form action="" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="nama" name="nama" required value="<?php echo htmlspecialchars($data_user['nama'] ?? ''); ?>"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="username" name="username" required value="<?php echo htmlspecialchars($data_user['username'] ?? ''); ?>"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                    </div>
                    
                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Alamat Email Baru</label>
                        <input type="email" id="email" name="email" required value="<?php echo htmlspecialchars($data_user['email'] ?? ''); ?>"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                    </div>
                </div>

                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-1">Kata Sandi Baru (Kosongkan jika tidak ingin diubah)</label>
                    <input type="password" id="password" name="password" placeholder="••••••••" autocomplete="new-password"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                </div>

                <div class="pt-4 border-t border-gray-100 flex justify-end gap-2">
                    <a href="index.php?page=home" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg text-sm hover:bg-gray-50 transition-colors">Batal</a>
                    <button type="submit" name="simpan_profil" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 font-medium shadow-sm transition-colors">
                        Simpan Perubahan
                    </button>
                </div>
